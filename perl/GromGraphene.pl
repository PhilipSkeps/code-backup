...
sub main {
    ...
    open(TEST, "/home/pskeps/Downloads/charmm-gui-6525792709/gromacs/step3_input.pdb") || die "failed to open test";
...
sub pairs {
	...
	my @PairsNonSplit = uniq(@PairsNonUniq);
	my @Pairs;

	map {
	
		my ($tempVar1, $tempVar2) = split(/\s+/, $_);
		push(@Pairs, $tempVar1, $tempVar2);
...
sub grapheneBuild {
    ...
    open(LOGFILE,">file.log") || die "unable to open logfile";
...
sub printITP {
    ...
    open(OUT, ">NMAtest.itp") || die "This sucks man\n";
